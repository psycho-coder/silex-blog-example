...
!function(){$(".input-group.date").datepicker({format:"dd.mm.yyyy",autoclose:!0,language:"ru"}),$(document).on("click","[data-confirmation]",function(a){a.preventDefault();var b=$(this);bootbox.confirm($(b).data("confirmation"),function(a){1==a&&window.location.replace($(b).attr("href"))})})}();
!function(a){function b(b,c,d){var e={previewURL:"",paramName:"content"};this.options=a.extend({},e,c);var f=this.options.previewURL,g={};g[this.options.paramName]=b,a.ajax({url:f,type:"POST",dataType:"html",data:g}).done(d)}a(document).on("change","#md-source textarea",function(b){a(b.target).addClass("changed")}),a(document).on("show.bs.tab",'a[href="#md-preview"]',function(){var c=a("#md-preview"),d=a("#md-source textarea");(d.hasClass("changed")||d.val().length>0&&0==c.html().length)&&(c.html('<img src="/img/ajax-loader.gif" class="md-loader" alt="" />'),b(d.val(),{previewURL:d.data("preview-url")},function(b){c.html(b),d.removeClass("changed"),a("pre code").each(function(a,b){hljs.highlightBlock(b)})}))})}(jQuery);